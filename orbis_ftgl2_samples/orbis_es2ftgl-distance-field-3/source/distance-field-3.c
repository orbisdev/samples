...
texture_font_t * font = 0;  // font is needed on render(): don't delete it!
...
void render( void )
{
  ...
  /*float glyph_height = glyph->height * width/(float)glyph->width;
    ...
    glUseProgram( program );
    {
        ...
        glUniformMatrix4fv( glGetUniformLocation( program, "u_model" ),  1, 0, model.data);
        glUniformMatrix4fv( glGetUniformLocation( program, "u_view" ),    1, 0, view.data);
        glUniformMatrix4fv( glGetUniformLocation( program, "u_projection" ),   1, 0, projection.data);
...
// ------------------------------------------------------------- load_glyph ---
texture_glyph_t *
load_glyph( const FT_Byte *buffer, size_t bufsize, const char *  codepoint,
            ...
            const float    padding )
{
    ...
    debugNetPrintf(DEBUG,"highres_data at %p\n", highres_data);
    ...
    debugNetPrintf(DEBUG,"Compute distance map\n");
    ...
    debugNetPrintf(DEBUG,"Allocate low resolution buffer\n");
    ...
    // Create glyph
    debugNetPrintf(DEBUG,"Create glyph\n");
...
//  font = texture_font_new_from_file( atlas, 32, filename );

    texture_glyph_t *glyph;

    // Generate the glyp at 256 points, compute distance field and scale it
    // back to 32 points
    // Just load another glyph if you want to see difference (draw render a '@')
    glyph = load_glyph( ttf, _orbisFile_lastopenFile_size, "@", 256, 64, 0.1);
    ...
    #else
    /* load from HOST0 */
    program = orbisGlCreateProgram("host0:shaders/distance-field.vert",
                                   "host0:shaders/distance-field-2.frag");
    #endif
    if(!program)
    {
